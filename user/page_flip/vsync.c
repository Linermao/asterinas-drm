...
static void modeset_draw(int fd)
{
	...
	while (time(&cur) < start + 5) {
		...
		if (ret < 0) {
			...
			int result = drmHandleEvent(fd, &ev);
			...
			// 			printf("  sequence: %u, tv_sec: %u, tv_usec: %u, crtc: %u\n",
			// 					vblank->sequence, vblank->tv_sec, vblank->tv_usec, vblank->crtc_id);
			// 			break;
			// 		case DRM_EVENT_FLIP_COMPLETE:
			// 			vblank = (struct drm_event_vblank *) e;
			// 			printf("  sequence: %u, tv_sec: %u, tv_usec: %u, crtc: %u\n",
			// 					vblank->sequence, vblank->tv_sec, vblank->tv_usec, vblank->crtc_id);
			// 			printf("page_flip_received %llu\n", vblank->user_data);
			// 			break;
			// 		case DRM_EVENT_CRTC_SEQUENCE:
			// 			seq = (struct drm_event_crtc_sequence *) e;
			// 			printf("  sequence: %llu, time_ns: %llu, user_data: %llu\n",
			// 					seq->sequence, seq->time_ns, seq->user_data);
			// 			break;
			// 	}
			...
			// 	modeset_draw_dev(fd, modeset_list);
		}
	}
}
