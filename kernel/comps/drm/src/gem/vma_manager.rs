...
impl Default for DrmVmaOffsetManager {
    fn default() -> Self {
...
impl DrmVmaOffsetManager {
    ...
    pub fn add(&self, gem_object: &Arc<dyn DrmGemObject>) -> Result<()> {
        ...
        inner.objects_by_start.insert(
            allocation.start_page,
            DrmVmaOffsetObject {
                gem_object: Arc::downgrade(gem_object),
                size_pages: allocation.size_pages,
            },
        );
        Ok(())
    }
    ...
    pub fn lookup(&self, start_page: u64, pages: u64) -> Option<Arc<dyn DrmGemObject>> {
        ...
        let mut inner = self.inner.lock();
        let (node_start, object) = inner
            .objects_by_start
            .range(..=start_page)
            .next_back()
            .map(|(start, object)| (*start, object.clone()))?;
        let gem_object = match object.gem_object.upgrade() {
            Some(gem_object) => gem_object,
            None => {
                inner.objects_by_start.remove(&node_start);
                inner.free_range(node_start, object.size_pages);
                return None;
            }
        };
        let node = gem_object.vma_node();
...
struct DrmVmaOffsetManagerInner {
    ...
    objects_by_start: BTreeMap<u64, DrmVmaOffsetObject>,
}

impl DrmVmaOffsetManagerInner {
    fn reclaim_dead_objects(&mut self) {
        let dead_objects = self
            .objects_by_start
            .iter()
            .filter(|(_, object)| object.gem_object.strong_count() == 0)
            .map(|(start_page, object)| (*start_page, object.size_pages))
            .collect::<Vec<_>>();

        for (start_page, size_pages) in dead_objects {
            self.objects_by_start.remove(&start_page);
            self.free_range(start_page, size_pages);
        }
    }

    fn allocate_best_fit(&mut self, pages: u64) -> Result<DrmVmaAllocation> {
        let Some((best_start, best_len)) = self.find_best_fit(pages) else {
            return_errno_with_message!(Errno::ENOMEM, "no free VMA offset range is large enough");
        };

        self.free_ranges.remove(&best_start);
        if best_len > pages {
            let Some(remaining_start) = best_start.checked_add(pages) else {
                return_errno_with_message!(Errno::ENOMEM, "the VMA offset range overflowed");
            };
...
#[derive(Clone, Debug)]
struct DrmVmaOffsetObject {
    gem_object: Weak<dyn DrmGemObject>,
    size_pages: u64,
}

/// Represents the mmap offset metadata attached to one GEM object.
///
/// The node does not own the object itself. The manager keeps a weak object
/// reference and lazily reclaims the allocated range on a later manager
/// operation after the last strong reference disappears.
#[derive(Debug)]
pub struct DrmVmaOffsetNode {
    allocation: Mutex<Option<DrmVmaAllocation>>,
    allowed_files: Mutex<BTreeMap<u64, usize>>,
}
...
impl DrmVmaOffsetNode {
    ...
    pub fn allow(&self, client_id: u64) -> Result<()> {
        let mut allowed_files = self.allowed_files.lock();
        let count = allowed_files.entry(client_id).or_insert(0);
        let Some(new_count) = count.checked_add(1) else {
            return_errno_with_message!(Errno::ENOMEM, "the VMA access reference count overflowed");
        };
        *count = new_count;
        Ok(())
    }
    ...
    pub fn allow_once(&self, client_id: u64) -> Result<()> {
        let mut allowed_files = self.allowed_files.lock();
        allowed_files.entry(client_id).or_insert(1);
        Ok(())
    }
    ...
    pub fn revoke(&self, client_id: u64) {
        let mut allowed_files = self.allowed_files.lock();
        let Some(count) = allowed_files.get_mut(&client_id) else {
            return;
        };

        *count -= 1;
        if *count == 0 {
            allowed_files.remove(&client_id);
        }
    }

    /// Returns whether the given open file is currently allowed to map this node.
    pub fn is_allowed(&self, client_id: u64) -> bool {
        self.allowed_files.lock().contains_key(&client_id)
    }
...
fn pages_from_size(size: usize) -> Result<u64> {
    if size == 0 {
        return_errno_with_message!(Errno::EINVAL, "the GEM object size must not be zero");
    }

    let Some(aligned_size) = size.checked_add(PAGE_SIZE - 1) else {
        return_errno_with_message!(Errno::ENOMEM, "the page-aligned GEM object size overflowed");
    };
    let aligned_size = aligned_size / PAGE_SIZE;
    let Ok(pages) = u64::try_from(aligned_size) else {
        return_errno_with_message!(Errno::ENOMEM, "the GEM object page count overflowed");
    };
    Ok(pages)
}
