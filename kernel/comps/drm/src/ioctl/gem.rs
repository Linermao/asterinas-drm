...
impl DrmFile {
    pub(super) fn drm_gem_close(&self, cmd: DrmIoctlGemClose) -> Result<i32> {
        let args = cmd.read()?;
        if args.pad != 0 {
            return_errno_with_message!(Errno::EINVAL, "the MAP_DUMB padding must be zero");
        }
    ...
    pub(super) fn drm_mode_create_dumb(&self, cmd: DrmIoctlModeCreateDumb) -> Result<i32> {
        ...
        let gem_ops = self.device().gem_ops().ok_or(Errno::EOPNOTSUPP)?;
        let gem_object = gem_ops.create_dumb(args.width, args.height, args.bpp)?;
