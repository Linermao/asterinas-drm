...
impl DrmFile {
    pub(crate) fn drm_get_version(&self, cmd: DrmIoctlVersion) -> Result<i32> {
        ...
        cmd.with_data_ptr(|args_ptr| {
            ...
            args.name_len = copy_drm_field(&userspace, args.name, args.name_len, name.as_bytes())?;
            args.date_len = copy_drm_field(&userspace, args.date, args.date_len, date.as_bytes())?;
            args.desc_len = copy_drm_field(&userspace, args.desc, args.desc_len, desc.as_bytes())?;
