use alloc::{boxed::Box, collections::VecDeque, fmt, vec::Vec};
use core::{
    fmt::Debug,
    mem::size_of,
...
pub trait VblankCallback: Debug + Send + Sync {
    /// Send vblank event data to userspace
    fn send_vblank_event(&self, data: &[u8]);
}
...
impl DrmPendingVblankEvent {
    ...
    pub fn new(
        ...
        let drm_event = DrmEvent {
            type_: DrmEventType::FlipComplete as u32,
        ...
        let drm_pending_event = DrmPendingEvent::new(drm_event)?;
        Ok(Self {
            drm_pending_event,
            pipe: 0,
            sequence: 0,
...
impl DrmPendingEvent {
    pub fn new(drm_event: DrmEvent) -> Result<Self, DrmError> {
        Ok(Self { drm_event })
    }
}
