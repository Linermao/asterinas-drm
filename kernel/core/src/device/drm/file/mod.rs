...
use aster_drm::device::{DrmDevice, DrmDeviceCapFlags, DrmFeatures};
use ostd::mm::VmIo;
...
struct DrmFileAuthState {
    ...
    was_master: bool,
    /// Stable authentication magic allocated on the first `GET_MAGIC` call.
    magic: Option<u32>,
}
...
impl DrmFile {
    pub(super) fn new(minor: Arc<DrmMinor>) -> Self {
        ...
        let is_master = if matches!(minor.type_(), DrmMinorType::Primary) {
            device.open_primary_client(client_id)
        } else {
            false
        };
...
impl Drop for DrmFile {
    fn drop(&mut self) {
        // Avoid file drop but the magic number has never been used.
        let mut auth_state = self.auth_state.lock();
        let magic = auth_state.magic.take();
        if let Some(magic) = magic {
            let _ = self.device.release_magic(magic);
        }
        drop(auth_state);
            ...
            match raw_ioctl {
                ...
                cmd @ DrmIoctlGetMagic => {
                    ...
                    let magic = match auth_state.magic {
                        Some(magic) => magic,
                        None => {
                            let magic = self.device.allocate_magic(&self.authenticated)?;
                ...
                cmd @ DrmIoctlAuthMagic => {
                    ...
                    self.device.authenticate_magic(args.magic)?;
                    Ok(0)
                }
                DrmIoctlSetMaster => {
                    ...
                    self.device.set_master(self.client_id)?;
