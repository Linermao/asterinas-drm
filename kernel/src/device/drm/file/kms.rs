...
impl DrmFile {
    ...
    pub(super) fn ioctl_mode_get_resources(&self, cmd: DrmIoctlModeGetResources) -> Result<i32> {
        ...
        cmd.with_data_ptr(|args_ptr| {
            ...
            let min_fb_rect = device.caps().min_fb_rect();
            let max_fb_rect = device.caps().max_fb_rect();

            args.min_width = min_fb_rect.width();
            args.min_height = min_fb_rect.height();
            args.max_width = max_fb_rect.width();
            args.max_height = max_fb_rect.height();
    ...
    pub(super) fn ioctl_mode_get_crtc(&self, cmd: DrmIoctlModeGetCrtc) -> Result<i32> {
        ...
        let (gamma_size, x, y, fb_id, mode) = {
            ...
            let crtc = objects
                ...
                .ok_or(Errno::ENOENT)?;
            let crtc_snapshot = crtc.state_snapshot();

            let primary_plane = objects
                .get_object::<DrmPlane>(crtc.primary_plane_id())
                .ok_or(Errno::ENOENT)?;
            let primary_plane_snapshot = primary_plane.state_snapshot();
    ...
    pub(super) fn ioctl_mode_get_encoder(&self, cmd: DrmIoctlModeGetEncoder) -> Result<i32> {
        ...
        let (crtc_id, encoder_type, possible_crtcs) = {
            ...
            let encoder = objects
                ...
                .ok_or(Errno::ENOENT)?;
            (
                encoder.state_snapshot().crtc_id().unwrap_or(0),
        ...
        cmd.with_data_ptr(|args_ptr| {
            let (
                ...
                let connector = objects
                    ...
                    .ok_or(Errno::ENOENT)?;
                let snapshot = connector.state_snapshot();
