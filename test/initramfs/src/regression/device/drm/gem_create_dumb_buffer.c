...
{
	uint64_t min_pitch =
		((uint64_t)create->width * create->bpp + 7) / 8;
	...
	return create->handle != 0 && create->pitch == min_pitch &&
		       create->size == expected_size && create->size <= SIZE_MAX
		       ? 0
		       : -1;
}
...
FN_TEST(create_map_destroy)
{
	...
	if (page_size > 0) {
		...
		int create_result = TEST_RES(
			ioctl(card_fd, DRM_IOCTL_MODE_CREATE_DUMB, &create),
			_ret == 0);

		if (create_result == 0) {
			int validation_result = TEST_RES(
				validate_created_buffer(&create, (size_t)page_size),
				_ret == 0);
			...
			if (validation_result == 0) {
				...
				int map_result = TEST_RES(
					ioctl(card_fd, DRM_IOCTL_MODE_MAP_DUMB,
					      &map),
					_ret == 0);

				if (map_result == 0 &&
				    TEST_RES(validate_mmap_offset(
						     &map, (size_t)page_size),
					     _ret == 0) == 0) {
					size_t map_size = (size_t)create.size;
					uint8_t *buffer = TEST_RES(
						mmap(NULL, map_size,
					...
					if (buffer != MAP_FAILED) {
						TEST_RES(verify_zeroed_buffer(
								 buffer, map_size),
							 _ret == 0);
						TEST_RES(write_and_verify_pattern(
								 buffer, map_size),
							 _ret == 0);
						TEST_RES(munmap(buffer, map_size),
							 _ret == 0);
					}
				}
			}
			...
			TEST_RES(ioctl(card_fd, DRM_IOCTL_MODE_DESTROY_DUMB,
				       &destroy),
				 _ret == 0);
		}
	}
